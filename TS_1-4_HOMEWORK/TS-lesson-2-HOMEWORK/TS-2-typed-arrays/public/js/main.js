...
console.group('Užduotys');
{
    ...
    {
        const stringsArr = ['Kaštonas', 'Alksnis', 'Uosis', 'Liepa', 'Alyvmedis', 'Skroblas'];
        function treeString(treeArr) {
            const firstLetterArr = treeArr.map((singleTree) => singleTree[0]);
            const lettersStr = firstLetterArr.reduce((previousValue, currentValue) => previousValue + currentValue);
    ...
    {
        const numbersArr = [1, 4, 1, 2, 1, 10];
        function numbersCount(numArr) {
            const numResult = numArr.reduce((previousValue, currentValue) => previousValue * currentValue);
