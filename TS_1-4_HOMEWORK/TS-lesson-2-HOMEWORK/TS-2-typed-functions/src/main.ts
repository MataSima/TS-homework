...
console.group('1. Aprašykite funkcijoms tipus jas deklaruojant');
{
  ...
  const power = function (base:number, powered :number):number {
    return base ** powered;
  };
...
console.group('2. Aprašykite funkcijų tipus prieš deklaruojant funkcijas');
{
  ...
  type Power = (a:number, b: number) => number;
  const power: Power = function (base, powered) {
    return base ** powered;
  };

  type SquareRoot = (a: number) => number;
  const squareRoot: SquareRoot = (number) => number ** (1 / 2);

  type Root = (a:number, b:number)=> number;
  const root: Root = function (base, nthRoot) {
    return base ** (1 / nthRoot);
  };

  type PrintBlueText = (a:string) => void;
