...
// Išvardinimas aukčio matavimo vienetams:
enum HeightUnits {
  CENTIMETRES = 'cm',
  METRES = 'm',
  INCHES = 'in',
...
// Klasė  sukurianti/koreguojanti asmens objektą:
class Person {
  ...
  public setAge(age:number) {
    if (age % 1 !== 0) throw new Error('Amžius privalo būti sveikasis skaičius');
    if (age < 1) throw new Error('Esate per jaunas arba dar negimęs');
    if (age > 150) throw new Error('Esate per senas arba jau miręs');
    this.age = age;
  }

  public setHeight(height:number, heightUnits: HeightUnits = HeightUnits.CENTIMETRES) {
    switch (heightUnits) {
  ...
  public setWeight(weight:number, weightUnits: WeightUnits = WeightUnits.KILOGRAMS) {
    switch (weightUnits) {
...
console.groupCollapsed('2. Sukurkite Person klasei savybę "age". Inkapsuliuokite šią savybę taip, jog reikšmė galėtų būti tik sveiki skaičiai nuo 1 iki 150');
{
  ...
  try { randomStrangerOne.setAge(-1); } catch (error) { console.error((error as Error).message); }
  console.log('Random person teo age is set in unrealistic value:');
  try { randomStrangerTwo.setAge(151); } catch (error) { console.error((error as Error).message); }
}
console.groupEnd();
